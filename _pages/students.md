---
layout: archive
title: "Students"
permalink: /members/students/
author_profile: false
classes: wide
---


## Graduate Students

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 20px; margin-bottom: 40px;">
  <!-- 학생 1 카드 샘플 -->
  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/woojin_cho.jpg" 
       alt="Woojin Cho" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">조우진(Cho, Woojin)</h3>
    <p style="font-size: 0.85rem; color: #718096; margin: 0 0 10px 0;">Ph.D. Student</p>
        <p style="font-size: 0.85rem; color: #718096; margin: 0 0 10px 0;">(2026.3 – Present)</p>
    <a href="mailto:wooju_1@kiel.re.kr" style="color: #2b6cb0; text-decoration: none;">wooju_1@kiel.re.kr</a>
  </div>
</div>

---

## Ubdergraduate Students

<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 20px; margin-bottom: 40px;">
  <!-- 학생 2 카드 샘플 -->
  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/juwon_min.jpg" 
       alt="juwon_min" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">민주원(Min, Juwon)</h3>
    <a href="mailto:minjoowon@gmail.com" style="color: #2b6cb0; text-decoration: none;">minjoowon@gmail.com</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/chaewon_yun.jpg" 
       alt="chaewon_yun" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">윤채원(Yun, Chaewon)</h3>
    <a href="mailto:codnjs026@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">codnjs026@catholic.ac.kr</a>
  </div>


  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/joonyoung_yoo.jpg" 
       alt="Joonyoung Yoo" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">유준영(Yoo, Joon Young)</h3>
    <a href="mailto:junriver63@gmail.com" style="color: #2b6cb0; text-decoration: none;">junriver63@gmail.com</a>
  </div>


  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/yoonseo_lee.jpg" 
       alt="Yoonseo Lee" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">이윤서(Lee, Yoon Seo)</h3>
    <a href="mailto:leeyoonseo@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">leeyoonseo@catholic.ac.kr</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/hyunjae_park.jpg" 
       alt="Park Hyunjae" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">박현재(Park, Hyun Jae)</h3>
    <a href="mailto:guswo4428@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">guswo4428@catholic.ac.kr</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/gyeol_koh.jpg" 
       alt="Koh Gyeol" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">고결(Koh, Gyeol)</h3>
    <a href="mailto:koh1565@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">koh1565@catholic.ac.kr</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/junyoung_ko.png" 
       alt="junyoung_ko" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">고준영(Ko, Junyoung)</h3>
    <a href="mailto:kojy1009@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">kojy1009@catholic.ac.kr</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/noimage.png" 
       alt="Noimg" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">조찬희(Jo, Chanhee)</h3>
    <a href="mailto:jch167@naver.com" style="color: #2b6cb0; text-decoration: none;">jch167@naver.com</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/taeyoung_hwang.jpeg" 
       alt="taeyoung_hwang" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">황태영(Hwang, Taeyoung)</h3>
    <a href="mailto:xodud0627@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">xodud0627@catholic.ac.kr</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/junoh_eom.jpg" 
       alt="Eom Junoh" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">엄준오(Eom, Junoh)</h3>
    <a href="mailto:joe081114@gmail.com" style="color: #2b6cb0; text-decoration: none;">joe081114@gmail.com</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/hyunjin_hwang.jpg" 
       alt="Hyunjin Hwang" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">황현진(Hwang, Hyunjin)</h3>
    <a href="mailto:lucy7444@catholic.ac.kr" style="color: #2b6cb0; text-decoration: none;">lucy7444@catholic.ac.kr</a>
  </div>

  <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; text-align: center; background: #ffffff; box-shadow: 0 2px 5px rgba(0,0,0,0.05);">
    <img src="/assets/images/hyunjune_ji.jpeg" 
       alt="hyunjune_ji" 
       style="width: 130px; height: 160px; object-fit: cover; margin: 0 auto 15px; border-radius: 6px; display: block; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    <h3 style="margin: 0 0 5px 0; font-size: 1.1rem;">지현준(Ji, Hyunjune)</h3>
    <a href="mailto:hyunjuneji@gmail.com" style="color: #2b6cb0; text-decoration: none;">hyunjuneji@gmail.com</a>
  </div>
</div>
---
